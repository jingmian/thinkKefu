...
{   
    ...
    public function index()
    {
        ...
        $this->assign([
            ...
            'word' => db('chat_statement')->where('status', 1)->select(),
            'groups' => db('chat_group')->where('status', 1)->select(),
            'status' => db('chat_kf_config')->where('id', 1)->find()
        ]);
    ...
    public function getUserList()
    {
        if(request()->isAjax()){
            ...
            $userList = db('chat_service_log')->field('user_id id,user_name name,user_avatar avatar,user_ip ip')
                ->where('kf_id', cookie('service_uid'))
    ...
    public function getChatLog()
    {
        if(request()->isAjax()){
            ...
            $logs = db('chat_log')->where(function($query) use($param){
                    $query->where('from_id', $param['uid'])->where('to_id', 'KF' . cookie('service_uid'));
            })->whereOr(function($query) use($param){
                $query->where('from_id', 'KF' . cookie('service_uid'))->where('to_id', $param['uid']);
            ...
            foreach($logs as $key=>$vo){
                ...
                if($vo['from_id'] == 'KF' . cookie('service_uid')){
                    $logs[$key]['type'] = 'mine';
                }
            }
    ...
    public function getCity()
    {
        ...
        if($info['city_id']){
            return json(['code' => 1, 'data' => $city['2'] . $city['3'] , 'msg' => 'ok']);
        }else{

            return json(['code' => 1, 'data' => $city['3'], 'msg' => 'ok']);
        }
    }
}
